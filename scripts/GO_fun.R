...
prep_goplot <- function(df, contrasts,
  ...
  ## Subset to focal contrasts
  df <- df %>%
    filter(contrast %in% contrasts,
           direction %in% directions)

  vars <- c("category", "ontology", "term",
            "contrast", "direction", "padj")

  if (pivot_by == "contrast") pivot_vals <- contrasts
  if (pivot_by == "direction") pivot_vals <- directions

  df %>%
    select(any_of(vars)) %>%
    ## Pivot wider and then longer to include all terms in all contrasts
    pivot_wider(names_from = .data[[pivot_by]], values_from = padj) %>%
    pivot_longer(cols = any_of(pivot_vals),
                 names_to = pivot_by, values_to = "padj") %>%
    left_join(df %>% select(contrast, category, term, direction, numDEInCat, sig),
              by = c("contrast", "category", "term", "direction")) %>%
    ...
    arrange(padj_log) %>%
    mutate(cat_term = str_trunc(paste0(category, " - ", term), width = 55))
}

## Heatmap-style plot for significant GO categories
GO_plot <- function(GO_res,
                    x_var = "timepoint", y_var = "term", fill_var = "padj_log",
                    textsize = 11, xlabs = NULL, label_count = FALSE) {

  p <- ggplot(data = GO_res) +
    aes_string(x_var, y_var, fill = fill_var) +
    geom_tile(stat = "identity", size = 0.25, color = "grey80") +
    #scale_fill_distiller(palette = "Blues", na.value = "grey20") +
